...
handler = logging.FileHandler(os.getcwd() + "/deletedFiles.log")
...
def cleanMedia():
    ...
    logger.info('Cron job ran at {}'.format(now))
    ...
    for filename in os.listdir(MEDIA_DIR):
        ...
        if os.path.isfile(filepath) and (now - os.stat(filepath).st_mtime) > (DAYS_THRESHOLD * 86400):
            ...
            logger.info('Deleting file {} with date {}'.format(filename, file_date))
            os.remove(filepath)

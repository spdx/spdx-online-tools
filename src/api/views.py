...
from api.serializers import ValidateSerializer, ConvertSerializer, CompareSerializer, CheckLicenseSerializer, \
    ValidateSerializerReturn, ConvertSerializerReturn, CompareSerializerReturn, CheckLicenseSerializerReturn
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                result = jpype.JavaException.message(
                    ex)  # + "This SPDX Document is not a valid RDF/XML or tag/value format"
                returnstatus = status.HTTP_400_BAD_REQUEST
                httpstatus = 400
                jpype.detachThreadFromJVM()
            except:
...
def convert(request):
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            query.status = httpstatus
            query.result = result
            ConvertFileUpload.objects.filter(file=uploaded_file).update(tagToRdfFormat=tagToRdfFormat, message=message,
                                                                        status=httpstatus, result=result)
...
def compare(request):
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            query.result = result
            query.status = httpstatus
            CompareFileUpload.objects.filter(file1=uploaded_file1).filter(file2=uploaded_file2).update(message=message,
                                                                                                       result=result,
                                                                                                       status=httpstatus)
